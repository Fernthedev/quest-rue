...
function stringToBytes(input: string, typeInfo: PacketJSON<ProtoTypeInfo>) {
    ...
    if (typeInfo.primitiveInfo != undefined) {
        ...
        case ProtoTypeInfo.Primitive.INT:
            new DataView(dataArray.buffer).setInt32(0, Number(input));
            break;
        case ProtoTypeInfo.Primitive.LONG:
            new DataView(dataArray.buffer).setBigInt64(0, BigInt(input));
            break;
        case ProtoTypeInfo.Primitive.FLOAT:
            new DataView(dataArray.buffer).setFloat32(0, Number(input));
            break;
        case ProtoTypeInfo.Primitive.DOUBLE:
            new DataView(dataArray.buffer).setFloat64(0, Number(input));
            break;
        case ProtoTypeInfo.Primitive.STRING:
            console.log("conveting %s to bytes", input);
            ...
            console.log("result:", [...dataArray].join(","));
            break;
    ...
    } else if (typeInfo.classInfo != undefined) {
        const value = Number(input.substring(input.lastIndexOf("at ") + 3));
        new DataView(dataArray.buffer).setInt32(0, value);
    }
    return dataArray;
}
...
function bytesToRealValue(bytes: DataView, typeInfo: PacketJSON<ProtoTypeInfo>, baseOffset: number) {
    ...
    if (typeInfo.classInfo != undefined) {
        return `${typeInfo.classInfo.namespaze}::${typeInfo.classInfo.clazz} at ${bytes.getInt32(baseOffset).toString()}`;
    } else if (typeInfo.structInfo != undefined) {
    ...
    // check for primitive last since it seems to sometimes be 0 instead of undefined
        ...
        case ProtoTypeInfo.Primitive.INT:
            return bytes.getInt32(baseOffset);
        case ProtoTypeInfo.Primitive.LONG:
            return bytes.getBigInt64(baseOffset);
        case ProtoTypeInfo.Primitive.FLOAT:
            return bytes.getFloat32(baseOffset);
        case ProtoTypeInfo.Primitive.DOUBLE:
            return bytes.getFloat64(baseOffset);
        case ProtoTypeInfo.Primitive.STRING:
