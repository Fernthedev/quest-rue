...
void WebSocketHandler::listen(int const port) {
    ...
    try {
        ...
        // }
        if (ec.failed()) {
            LOG_INFO("Failed to listen after trying v4: {} ({})", ec.message(), ec.to_string());
            return;
        }
        ...
        if (ec.failed()) {
            ...
            if (endpoint.address().is_v4()) {
                LOG_INFO("IPv4 {}:{}", endpoint.address().to_v4().to_string(ec), endpoint.port());
            }

            if (endpoint.address().is_v6()) {
                auto ipv6 = endpoint.address().to_v6();
                LOG_INFO("IPv6 {}:{} ipv4 compatible {}", ipv6.to_string(ec), endpoint.port(), ipv6.is_v4_compatible());
            }
        }
